      ...
      <Table className={classes.table} aria-label="custom pagination table">
        <TableHead>
          <TableRow>
            ...
            <TableCell width="12%">รายการที่ถูกแก้ไข</TableCell>
            <TableCell width="25%">ข้อมูลเดิม</TableCell>
            <TableCell width="25%">ข้อมูลใหม่</TableCell>
            <TableCell>เวลาที่แก้ไข</TableCell>
        ...
        <TableBody>
          {(rowsPerPage > 0 ? props.rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : props.rows).map(
            (row, index) => {
              const key = [];
              for (let k in row.editedItem) {
                key.push(k);
              }
              return (
                <TableRow key={row.name}>
                  <TableCell className={classes.tableCell} align={"center"}>{(index + 1) + (rowsPerPage*page)}</TableCell>
                  ...
                  <TableCell className={classes.tableCell}>{row.user}</TableCell>
                </TableRow>
              );
            }
          )}
